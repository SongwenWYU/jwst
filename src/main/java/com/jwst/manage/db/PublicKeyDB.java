...
import com.jwst.manage.model.PublicKey;
...
//    }
    ...
    public static SortedSet getPublicKeySet(SortedSet sortedSet) {
        ...
        String sql = "select * from public_keys where ";

//        sql+= StringUtils.isNotEmpty(sortedSet.getFilterMap().get(FILTER_BY_USER_ID)) ? " and p.user_id=? " : "";
        sql+= StringUtils.isNotEmpty(sortedSet.getFilterMap().get(FILTER_BY_PROFILE_ID)) ? " and profile_id=? " : "";
        sql+= StringUtils.isNotEmpty(sortedSet.getFilterMap().get(FILTER_BY_ENABLED)) ? " and enabled=? " : " and enabled=true";
        sql=sql+orderBy;
...
//    }
    ...
    public static void insertPublicKey(PublicKey publicKey) {
        ...
        try {
            con = DBUtils.getConn();
            PreparedStatement stmt = con.prepareStatement("insert into public_keys(key_nm, type, fingerprint, public_key, profile_id) values (?,?,?,?,?)");
    ...
    public static void updatePublicKey(PublicKey publicKey) {
        ...
        try {
            con = DBUtils.getConn();
            PreparedStatement stmt = con.prepareStatement("update public_keys set key_nm=?, type=?, fingerprint=?, public_key=?, profile_id=? where id=?  and enabled=true");
            stmt.setString(1, publicKey.getKeyNm());
            stmt.setString(2, SSHUtil.getKeyType(publicKey.getPublicKey()));
            stmt.setString(3, SSHUtil.getFingerprint(publicKey.getPublicKey()));
            stmt.setString(4, publicKey.getPublicKey().trim());
    ...
    public static void deletePublicKey(Long publicKeyId) {
        ...
        try {
            con = DBUtils.getConn();
            PreparedStatement stmt = con.prepareStatement("delete from public_keys where id=? and enabled=true");
            stmt.setLong(1, publicKeyId);
            stmt.execute();
            DBUtils.closeStmt(stmt);
