...
//		}
//		String sql = "select * from system where id in (select distinct system_id from  system_map m where m.profile_id=um.profile_id and um.user_id=? ";
...
//	}
	 ...
	 * @return system list
	 */
	public static HostSystem getAllSystemsByHostsystem(HostSystem hostSystem) {

		Connection con = null;
		HostSystem hostSystemResult = new HostSystem();
		try {
			con=DBUtils.getConn();
			PreparedStatement stmt = con.prepareStatement("select * from  system where user=?, host=?, port=?");
			stmt.setString(1, hostSystem.getUser());
			stmt.setString(2, hostSystem.getHost());
			stmt.setInt(3, hostSystem.getPort());
			ResultSet rs = stmt.executeQuery();
			
			if (rs.next()) {
				
				hostSystemResult.setId(rs.getLong("id"));
				hostSystemResult.setDisplayNm(rs.getString("display_nm"));
				hostSystemResult.setUser(rs.getString("user"));
				hostSystemResult.setHost(rs.getString("host"));
				hostSystemResult.setPort(rs.getInt("port"));
				hostSystemResult.setAuthorizedKeys(rs.getString("authorized_keys"));
				hostSystemResult.setStatusCd(rs.getString("status_cd"));
			}
			DBUtils.closeRs(rs);
			DBUtils.closeStmt(stmt);
